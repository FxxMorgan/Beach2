...
// Obtener datos de cheques mensuales
$cheques_mensuales = [];
$sql_cheques = "SELECT MONTH(fecha_pago) AS mes, SUM(monto) AS total FROM cheques WHERE sucursal_id = ? AND estado = 'listo' GROUP BY mes";
$stmt_cheques = $conn->prepare($sql_cheques);
$stmt_cheques->bind_param("i", $sucursal_id);
$stmt_cheques->execute();
$result_cheques = $stmt_cheques->get_result();
while ($row = $result_cheques->fetch_assoc()) {
    $cheques_mensuales[(int)$row['mes']] = (float)$row['total'];
}
...
<body>
    <div class="flex">
        ...
        <aside class="sidebar fixed h-screen w-64 p-4">
            ...
            <nav>
                <ul>
                    <li class="mb-4">
                        <a href="dashboard_jefe.php" class="flex items-center text-white hover:bg-blue-600 p-2 rounded">
        ...
        <main class="content flex-grow p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
                <div class="card p-6">
                    <h2 class="text-xl font-semibold mb-4">Ventas Mensuales</h2>
                ...
                <div class="card p-6">
                    <h2 class="text-xl font-semibold mb-4">Cheques Mensuales</h2>
                    <div class="chart-container">
                        <canvas id="chequesChart"></canvas>
                    </div>
                </div>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', (event) => {
            ...
            const datosCheques = meses.map((mes, index) => chequesMensuales[index + 1] || 0);
            ...
            // Inicializar Chart.js para Cheques
            var chequesCtx = document.getElementById('chequesChart').getContext('2d');
            var chequesChart = new Chart(chequesCtx, {
                type: 'pie',
                data: {
                    labels: meses,
                    datasets: [{
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: 'Distribución de Cheques Mensuales'
                        }
                    }
                }
            });
